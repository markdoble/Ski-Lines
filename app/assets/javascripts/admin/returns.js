$(document).ready(function(){
  ...
  $('.refund_select').change(function(event) {
        ...
        var defaultShipping = orderUnitShipping*qtyReturned;
        ...
        $("#refund_submit").val('Submit ' + "$"+defaultTotal.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') + " Refund");
    });
    ...
    $("#refund_submit").val('Submit ' + "$"+sum.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') + " Refund");
	}
