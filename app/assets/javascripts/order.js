...
/* GST only Terrotories */
	...
	/* Run change functions on page refresh to reset cart */
	$(function () {
	...
	/* Run prov/state change function when country select change */
...
/* unit selection change: set sub-total  */
...
/* Prov/state change function: caluculate shipping and tax based on customer's location */
	...
	/* Calculate Tax */
				/* Apply local tax if picking up in store. */
				if ($this.find('.in_store_delivery_method').prop('checked')){
				...
				/* Customr and Merchant in same non-HST jurisdiction */
...
/* Show comments form and hide unit selections form */
		...
		/* store dropdown selections for session */
