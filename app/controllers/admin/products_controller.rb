class Admin::ProductsController < ApplicationController
  ...
  def stock_product_upload
    ...
    # check to see if rep has selected a merchant or a session has been created
    if !params[:merchant_selected].blank? || !session[:merchant_selected].blank?
      # if a merchant was selected , create a session
      if !params[:merchant_selected].blank?
        session[:merchant_selected] = params[:merchant_selected]
      end
